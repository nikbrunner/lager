...
use crate::application::ports::{ConfigStore, Mutation};
...
fn mutate<T>(
    ...
    let mut document = parse_document(&target, &content)?;
    warn_unknown_keys(&document);
    decode_config(&target, document.clone())?;
...
fn decode_config(path: &Path, document: DocumentMut) -> Result<Config, ConfigStoreError> {
    let config: Config =
        toml_edit::de::from_document(document).map_err(|source| ConfigStoreError::Parse {
            path: path.to_path_buf(),
...
fn parse_document(path: &Path, content: &str) -> Result<DocumentMut, ConfigStoreError> {
    content
        .parse::<DocumentMut>()
        .map_err(|source| ConfigStoreError::Parse {
            path: path.to_path_buf(),
            message: source.to_string(),
        })
}
...
fn warn_unknown_keys(document: &DocumentMut) {
    for (key, _) in document.iter() {
        if !matches!(key, "root" | "providers" | "repositories") {
            eprintln!("lager: warning: unknown config key `{key}`");
        }
    }
            ...
            if let Some(table) = item.as_table_like() {
                for (key, _) in table.iter() {
                    if !matches!(
                        ...
                        eprintln!("lager: warning: unknown provider key `{host}.{key}`");
                    }
                }
            }
        }
        ...
        for (index, repository) in repositories.iter().enumerate() {
            for (key, _) in repository.iter() {
                if !matches!(key, "url" | "post_clone" | "exclude") {
                    eprintln!(
                        "lager: warning: unknown repository key `repositories[{index}].{key}`"
                    );
                }
            }
        }
