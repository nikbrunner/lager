...
fn inventory_advertises_only_offline_capabilities_and_rejects_remote_before_raw_mode() {
    ...
    for name in ["inventory", "inv"] {
        ...
        assert!(
            plain.contains("q quit") && plain.contains("R refresh"),
            "{plain}"
        );
        for unavailable in ["/ search", "all actions", "Add to", "Navigation is ready"] {
...
fn run_inventory_pty_with_exit(
    ...
    loop {
        ...
        if let Some((waits, input)) = actions.get(action_index)
            && waits
                .iter()
                .all(|text| screen_text(&transcript, rows as usize, cols as usize).contains(text))
        {
