...
- [ ] Open Inspect on each alpha row, the originless row, dirty checkout and wildcard pattern. Scroll to full paths, relationships, escaped warnings and unavailable-action reasons. Close with q or Esc to the same selected row; Ctrl-Q quits the app from the popup. Explicit alpha remains explicit even though a wildcard exclusion is also listed.
